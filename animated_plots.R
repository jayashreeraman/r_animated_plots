...
saveGIF({
  for (year in unique(india_us_forex$DATE)){
    ...
    p_forex <- ggplot(data_yearly, aes(x=DATE, y=AEXINUS)) +
      ...
      ylim(1,100) +ggtitle(year)
    
    print(p_forex)
    
}}, movie.name = "./Viz/india_us+forex_TS.gif", interval = 0.2)
...
saveGIF({
  for (year in unique(india_us_forex$DATE)){
    ...
    p_forex_lc <-ggplot(data_yearly, aes(DATE, AEXINUS)) + geom_line(group=1, color="maroon", aes(size=0.01))+ 
      theme(axis.text.x  = element_text(angle = 90, hjust = 1)) + 
  ...
  }}, movie.name = "./Viz/india_us+forex_TS_LineChart.gif", interval = 0.2)
